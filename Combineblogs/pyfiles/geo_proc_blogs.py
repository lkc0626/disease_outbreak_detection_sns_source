...
def geo_proc_blogs_f(key_terms, folder, path,co1):
    ''' 
    ...
    for file in files[:]:
        ...
        for line in open(os.path.join(folder, file)).readlines():
            ...
            dt = datetime.datetime.strptime(blog['date'][:10], "%Y-%m-%d")
            kws = {kw: 1 for kw in blog['content'].split() if kw in key_terms}
            geo = blog['embersGeoCode']
            for ci in geo:        
               ci = geo['city']
            for co in geo:
               co = geo['country']
    ...
    #    database = pickle.loads(open('data/{0}_ci_count_data.txt'.format(co1)).read())
        ...
        gsrf=ofolder+'/{0}_blogs_cnt_data.txt'.format(co1)
