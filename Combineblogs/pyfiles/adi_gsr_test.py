...
def deletePreviousOutput(path):
	for file in os.listdir(path):
			file_path = os.path.join(path+"/input/gsr_pkl", file)
			try: 
...
def generateGsrPkl(path, gsr):	
	for key in gsr: 
		...
		pathName = os.path.join(path+"/input/gsr_pkl/", str(key[1])+'_'+str(key[3])+'_gsr.txt')
			...
			if ci_2_id.has_key(ci):
				...
				for dt in sorted(dts):
					if dt >= cutdt and dt <= enddt: 
						...
						pathName = os.path.join(path+"/input/gsr_cutoff", 'Gsr_'+co+'_'+et+'.txt')
