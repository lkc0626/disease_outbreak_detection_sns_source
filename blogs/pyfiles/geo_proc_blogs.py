...
vocab = json.loads(open('hantavirus_keywords.txt').read())
...
for kw, i in vocab.items():
    ...
    if flag == 1 or kw.find('@') >= 0 or kw.find('#') >= 0:
        new_terms.append(kw)
'''
vocab = {t: 1 for t in key_terms}

database = dict()
#for co1 in cos[4:]:
folder = 'dataset/blogs-2012'
...
open('data/{0}_blogs_cnt_data.txt'.format('chile'), 'w').write(pickle.dumps(database))
database = pickle.loads(open('data/{0}_blogs_cnt_data.txt'.format('chile')).read())
print database 
