...
def makeWarningFile(path,wpath):
    warningPath = os.path.join(wpath, 'warning_file.txt')	
    warning_file = open(warningPath, 'w+')
    warning_file.seek(0)
...
def adi_fp_tp_calc_f(cutoff_score,co,et):
    path='../input'
    wf='../input/warning_file.txt'
    subgraph='../output/subraph'
    makeWarningFile(subgraph,path)

    Ground_truth='Gsr_'+co+'_'+et+'.txt'
    GT=path+'/'+Ground_truth
    proc(wf,GT,cutoff_score)	
'''
