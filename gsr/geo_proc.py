...
vocab = json.loads(open('data/cu_keys.txt').read())
...
#    database = pickle.loads(open('data/{0}_ci_count_data.txt'.format(co1)).read())
    open('data/{0}_cnt_data.txt'.format('chile'), 'w').write(pickle.dumps(database))
database = pickle.loads(open('data/{0}_cnt_data.txt'.format('chile')).read())
print database 
