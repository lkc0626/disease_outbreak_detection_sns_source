...
def getinfnpss_f(alpha_max,folder,folder1,graphfile):
...
#    files = [file for file in os.listdir(folder) if file.find(co1) >= 0]
   ...
   #     print file
        ...
        if not os.path.exists(folder1):
            os.makedirs(folder1)
        out = open(os.path.join(folder1, '{0}'.format(file)), 'w')
...
if __name__ =='__main__':
  ...
  #  network = {'0_1': 0.2, '1_2': 0.1, '2_0': 0.2, '2_3': 0.2, '3_4': 0.2}
    alpha_max = 1
    folder = 'twitter_output'
    folder1 = 'twitter_subgraph'
    graphfile='chile_k_25_graph.txt'
    getinfnpss_f(alpha_max,folder,folder1,graphfile)
  #  print pvalue.items()[0][1]
  #  print npss_detection(pvalue.items()[0][1], network, alpha_max)
    '''
    pvalue = {}
    for line in open('2012-08-27.txt').readlines():
        file='2012-08-27.txt'
      #  pvalue={}
      #  pvalue['2012-08-27.txt']=
        item = line.split()
...
#    files = [file for file in os.listdir(folder) if file.find(co1) >= 0]
   ...
   #     print file
        ...
        network = {}
        for line in open('chile_k_25_graph.txt').readlines():
            item = line.split()
            edge = '{0}_{1}'.format(item[0], item[1])
            network[edge] = float(item[2])
